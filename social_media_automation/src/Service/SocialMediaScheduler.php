...
class SocialMediaScheduler {
   ...
   * Now supports 9 posts per day every 2 hours from 6 AM to 10 PM.
   */
  public function executeCronPosts(): void {
    ...
    // Nine daily posts: Every 2 hours from 6 AM to 10 PM
    $posting_windows = [
      'early_morning' => ['start' => 6, 'end' => 8, 'state_key' => 'last_early_morning_post'],
      'morning' => ['start' => 8, 'end' => 10, 'state_key' => 'last_morning_post'],
      'mid_morning' => ['start' => 10, 'end' => 12, 'state_key' => 'last_mid_morning_post'],
      'noon' => ['start' => 12, 'end' => 14, 'state_key' => 'last_noon_post'],
      'afternoon' => ['start' => 14, 'end' => 16, 'state_key' => 'last_afternoon_post'],
      'late_afternoon' => ['start' => 16, 'end' => 18, 'state_key' => 'last_late_afternoon_post'],
      'evening' => ['start' => 18, 'end' => 20, 'state_key' => 'last_evening_post'],
      'late_evening' => ['start' => 20, 'end' => 22, 'state_key' => 'last_late_evening_post'],
      'night' => ['start' => 22, 'end' => 24, 'state_key' => 'last_night_post'],
    ];
  ...
  /**
   * Process social media interactions (mentions, boosts) for enabled platforms.
   * 
   * Called by cron to check for and respond to mentions and boosts.
   */
  public function processInteractions(): void {
    ...
    // Process interactions for each enabled platform that supports it
    $enabled_platforms = $config->get('enabled_platforms') ?? [];
    
    foreach ($enabled_platforms as $platform_name) {
      ...
      // Check if platform supports interaction processing
      if ($platform && method_exists($platform, 'processInteractions')) {
        try {
          $this->logInfo('Processing interactions for platform: @platform', ['@platform' => $platform_name]);
          $platform->processInteractions();
        } catch (\Exception $e) {
          $this->logError('Failed to process interactions for @platform: @message', [
  ...
  protected function queueScheduledPost(string $window_name): void {
    ...
    // ONLY use recent_article content type to ensure all posts are AI-generated 
    // with sports broadcaster style about specific articles
    $content_type = 'recent_article';
   ...
   */
  public function sendTestPost(string $content_type = 'recent_article'): array {
    $results = [];
   ...
   *   TRUE if successful, FALSE otherwise.
   */
  public function sendTestPostToPlatform(string $platform_name, string $content_type = 'recent_article'): bool {
    try {
      $platform = $this->platformManager->getPlatform($platform_name);
  ...
  public function forcePost(string $content_type = NULL): bool {
    ...
    // Determine content type - always use recent_article for AI-generated content
    if (!$content_type) {
      $content_type = 'recent_article';
    }
