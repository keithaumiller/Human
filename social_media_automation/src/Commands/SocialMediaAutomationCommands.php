...
class SocialMediaAutomationCommands extends DrushCommands {
  ...
  public function cronRun() {
    try {
      ...
      $this->output()->writeln('Daily posting windows: 6 AM - 12 AM (9 windows every 2 hours)');
      $this->output()->writeln('');
      ...
      // Execute cron - both posts and interactions
      $this->output()->writeln('<comment>Executing scheduled posts cron...</comment>');
      $this->scheduler->executeCronPosts();
      
      $this->output()->writeln('<comment>Processing social media interactions...</comment>');
      $this->scheduler->processInteractions();
      ...
      if ($queue_count_after > $queue_count_before) {
        ...
        // Define posting windows to check current status
        $posting_windows = [
          'early_morning' => ['start' => 6, 'end' => 8, 'name' => 'Early Morning'],
          'morning' => ['start' => 8, 'end' => 10, 'name' => 'Morning'],
          'mid_morning' => ['start' => 10, 'end' => 12, 'name' => 'Mid-Morning'],
          'noon' => ['start' => 12, 'end' => 14, 'name' => 'Noon'],
          'afternoon' => ['start' => 14, 'end' => 16, 'name' => 'Afternoon'],
          'late_afternoon' => ['start' => 16, 'end' => 18, 'name' => 'Late Afternoon'],
          'evening' => ['start' => 18, 'end' => 20, 'name' => 'Evening'],
          'late_evening' => ['start' => 20, 'end' => 22, 'name' => 'Late Evening'],
          'night' => ['start' => 22, 'end' => 24, 'name' => 'Night'],
        ];
        
        $in_window = FALSE;
        $current_window = '';
        foreach ($posting_windows as $window_key => $window) {
        ...
        if ($in_window) {
          $last_post_key = 'social_media_automation.last_' . str_replace(' ', '_', strtolower($current_window)) . '_post';
          $last_window_post = \Drupal::state()->get($last_post_key, 0);
          $last_window_post_date = $last_window_post ? date('Y-m-d', $last_window_post) : 'Never';
          
          if ($last_window_post_date === $today) {
            $this->output()->writeln('<comment>⏸️ ' . $current_window . ' post already sent today (' . $last_window_post_date . ').</comment>');
          } else {
            $this->output()->writeln('<comment>⚠️ In ' . $current_window . ' posting window but no post queued. Check platform configuration.</comment>');
          }
        } else {
          $this->output()->writeln('<comment>⏰ Outside posting window (current hour: ' . $current_hour . ').</comment>');
          
          // Find next window
          $next_window_hour = NULL;
          foreach ($posting_windows as $window) {
          ...
          if ($next_window_hour) {
            $hour_12 = ($next_window_hour == 0) ? '12 AM' : (($next_window_hour <= 12) ? $next_window_hour . ' AM' : ($next_window_hour - 12) . ' PM');
            $this->output()->writeln('Next posting window starts at ' . $hour_12 . '.');
          } else {
            $this->output()->writeln('Next posting window starts tomorrow at 6 AM.');
          }
        }
      }
  ...
  /**
   * Test interaction processing (mentions and boosts).
   * 
   * @command social-media:test-interactions
   * @aliases sma:interactions
   * @usage social-media:test-interactions
   *   Test processing of mentions, boosts, and responses on enabled platforms
   */
  public function testInteractions() {
    try {
      $config = \Drupal::config('social_media_automation.settings');
      
      $this->output()->writeln('<info>=== Testing Social Media Interactions ===</info>');
      $this->output()->writeln('');
      
      if (!$config->get('enabled')) {
        $this->output()->writeln('<error>Social media automation is disabled. Enable it in the admin settings first.</error>');
        return;
      }

      $this->output()->writeln('Checking for mentions, boosts, and interactions...');
      $this->output()->writeln('');
      ...
      if (empty($enabled_platforms)) {
        $this->output()->writeln('<error>No platforms are enabled. Configure at least one platform first.</error>');
        return;
      }

      foreach ($enabled_platforms as $platform) {
        $platform_name = strtolower($platform->getName());
        $this->output()->writeln("Testing interactions for {$platform->getName()}...");
        
        if (method_exists($platform, 'processInteractions')) {
          try {
            $platform->processInteractions();
            $this->output()->writeln("<info>✅ {$platform->getName()}: Interactions processed successfully</info>");
          } catch (\Exception $e) {
            $this->output()->writeln("<error>❌ {$platform->getName()}: Failed to process interactions</error>");
            $this->output()->writeln("<comment>Error: {$e->getMessage()}</comment>");
          }
        } else {
          $this->output()->writeln("<comment>⚠️  {$platform->getName()}: Interaction processing not supported</comment>");
        }
        
        $this->output()->writeln('');
      }
      
      $this->output()->writeln('<info>Interaction processing test complete!</info>');
      $this->output()->writeln('');
      $this->output()->writeln('<comment>Check logs for detailed interaction processing:</comment>');
      $this->output()->writeln('<comment>drush watchdog:show --filter=social_media_automation</comment>');
      
    } catch (\Exception $e) {
      $this->output()->writeln('<error>Error testing interactions: ' . $e->getMessage() . '</error>');
    }
  }
