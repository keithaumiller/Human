...
class AIApiService {
  ...
  public function sendMessage(NodeInterface $conversation, string $message) {
    try {
      ...
      // Get the AI model: conversation field → config → hardcoded default.
      $model = $conversation->get('field_ai_model')->value ?: $config->get('aws_model') ?: 'anthropic.claude-3-5-sonnet-20241022-v2:0';
  ...
  private function generateSummary(string $context) {
    try {
      ...
      $bedrock = $sdk->createBedrockRuntime();
      $model = $config->get('aws_model') ?: 'anthropic.claude-3-5-sonnet-20241022-v2:0';
